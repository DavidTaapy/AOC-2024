...
class Solution():
    ...
    def __init__(self):
        self.input_path: str = "./Day 8/Case 2/Input.txt"
    ...
    def _get_antinodes(self, loc_one: tuple[int, int], loc_two: tuple[int, int]) -> list[tuple[int, int]]:
        ...
        # Handle one way
        antinode_one = loc_one
        while self._is_in_boundary(antinode_one):
            result.append(antinode_one)
            antinode_one = (antinode_one[0] - x_diff, antinode_one[1] - y_diff)
        # Handle other way
        antinode_two = loc_two
        while self._is_in_boundary(antinode_two):
            result.append(antinode_two)
            antinode_two = (antinode_two[0] + x_diff, antinode_two[1] + y_diff)
        # Return result
        return result
