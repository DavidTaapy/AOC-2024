...
class Solution():
    ...
    def __init__(self):
        self.input_path: str = "./Day 5/Case 1/Input.txt"
    ...
    def solve(self, inputs: tuple[list[list[int]], list[list[int]]]) -> int:
        ...
        # Filter out unsafe updates
        safe_updates = [update for update in updates if self._is_safe_update(update, page_rule_dict)]
        # Get the result
        return sum(self._get_middle_number(safe_updates[i]) for i in range(len(safe_updates)))
